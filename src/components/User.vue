  ...
  <div>
    ...
    <div class="list">
      ...
      <div v-for="(item, index) in userList" :key="item.id" class="row">
        ...
        <div class="cell cell-time">{{item.nickname}}</div>
        <div class="cell cell-time">{{item.status}}</div>
    ...
    data() {
      return {
        ...
        fields: [
          ...
          { label: '状态', key: 'status', render:(val, obj)=>{} },
          { label: '注册时间', key: 'createTime', render:(val, obj)=>{} }
        ]
      }
    },
    created() {
